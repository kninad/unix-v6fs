...
#define LEN_FNAME 13  // One less than 14 to have space for the terminating null char. 
...
int main(int argc, char *argv) {
    ...
    printf("* Available commands are: initfs, cpin, cpout, cd, mkdir, rm, pwd, ls, q\n");    
    printf("* Enter the required commands below:\n\n");

    bool counter = false; // whether initfs was the first program or not?
    ...
    while (1) {
        ...
        if (strcmp(splitter, "initfs") == 0) {
            ...
            printf("First command on startup has to be initfs!\n");
            ...
            if (!arg1 || !arg2) {
                ...
                if (ret <= 0) {
                    printf(" cpin operation failed!\n\n");
                } else {
                    printf(" cpin operation was successful.\n\n");
                }
            }
            ...
            if (!arg1 || !arg2) {
                ...
                if (ret <= 0) {
                    printf(" cpout operation failed! \n\n");
                } else {
                    printf(" cpout operation was successful.\n\n");
                }
            }
...
void list_pwd() {
    ...
    printf("Note: Any free space in between entries will also show up!\n");
    // printf("Contents: ");
    // Move to appropriate position in root dir's last data block and write out info
    dir_type tmp_buffer;
    for (int i = 0; i < ADDR_SIZE; ++i) {
        ...
        while (read_bytes > 0 && count <= BLOCK_SIZE) {
            ...
            printf("\t%s (%d)\n", tmp_buffer.filename, tmp_buffer.inode);
            read_bytes = read(fileDescriptor, &tmp_buffer, sizeof(dir_type));
            count += read_bytes;
        }
    }
...
int copy_in_2(char *external_file, char *v6_filename) {
    ...
    if (strlen(final_file) > LEN_FNAME) {
...
int remove_file(char *v6_file) {
    ...
    if (file_inode_num < 2) {
        ...
        if(!is_empty_dir(file_inode_num)){
            ...
            printf(" Note: Given path is an empty directory. It will be now deleted!\n");
        } 
    }
...
int make_dir(char *path) {
    ...
    printf(" Created new dir with inode: %d\n", dir_inode_num);
    return 1;
}
