...
int addFile(unsigned short inodeN, unsigned char* path, unsigned char* filename){
	int i,j, address = 0;
	unsigned short workInodeN = inodeNOf(path);
	...
	if(address!=0){
		...
		//printf("%d %d %d %d\n", workInode.size, workInode.addr[0], workInodeN, addressOfInode(workInodeN));
...
/*
 * checks if the inode stored in 'inode' is a file or an empty directory
 */ 
int canDeleteWorkInode(){
	//can delete if it is a directory, or if its size is 32 (meaning only two entries)
	//return !(inode.flags&dir_flag) || inode.size <= 32;
...
int rmInit(){
	...
	if(strcmp(path,"/") == 0 || strcmp(path,".") == 0 || strcmp(path,"..") == 0){
		printf("\tCannot delete the paths give by /, . or ..\n");
		return -2;
	}
	...
	//if not found
	if(inodeN==0 || address == 0){
		printf("\tFile not found\n");
		return -3;
	}
	
	//if attempting to delete root
	if(inodeN == 1){
		printf("\tCannot delete root directory\n");
		return -2;
	}
	
	//delete the file with inodeN
	//getting the inode of the erased file
	lseek(fileDescriptor, addressOfInode(inodeN), SEEK_SET);
	read(fileDescriptor, &inode, INODE_SIZE);
	...
	lseek(fileDescriptor, address, SEEK_SET);
	write(fileDescriptor, &dir, 16);
	//size of parent decreases by 16
	workInode.size -= 16;
	lseek(fileDescriptor, addressOfInode(parentInodeN), SEEK_SET);
	write(fileDescriptor, &workInode, INODE_SIZE);

	//we need to deallocate it;
...
/*
 * refresing the inodes in memory
 */
void refreshCurInode(){
	lseek(fileDescriptor, addressOfInode(curInodeN), SEEK_SET);
	read(fileDescriptor, &curInode, INODE_SIZE);
	lseek(fileDescriptor, addressOfInode(1), SEEK_SET);
